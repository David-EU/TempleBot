...
def pawnstrip(character, server, region):
    try:
        with open('%s%s-%s-%s-1.html' % (simcraft_path, character, server, region), encoding='utf8') as infile:
            soup = BeautifulSoup(infile, "html.parser")
            return soup.find(text=re.compile(' Pawn: v1: '))
    except:
        print('Bad sim, cannot find %s%s-%s-%s-1.html' % (simcraft_path, character, server, region))
...
async def on_ready():
        ...
        await client.send_message(client.get_channel(channel), '%s: Stat weight simulation on %s completed. This is for 1 target (same as default sim)' % (author, character))
        ...
        subprocess.Popen('python3 sim2.py %s %s %s %s %s no' % (character, server, channel, escapeAuthor, region), shell=True)
        await client.logout()
print('Starting sim:')
print('%s./simc armory=%s,%s,%s calculate_scale_factors=1 scale_only=intellect,crit_rating,haste_rating,mastery_rating,versatility_rating iterations=10000 html=%s-%s-%s-1.html output=%s-%s-1.txt' % (simcraft_path, region, server, character, character, server, region, character, region))
subprocess.call('%s./simc armory=%s,%s,%s calculate_scale_factors=1 scale_only=intellect,crit_rating,haste_rating,mastery_rating,versatility_rating iterations=10000 html=%s-%s-%s-1.html output=%s-%s-1.txt' % (simcraft_path, region, server, character, character, server, region, character, region), cwd=simcraft_path, shell=True)
client.run(token)